import { getHillsReceipt } from "@/api/get-hills-receipt";
import { CardSkeleton } from "@/components/card-skeleton";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { useQuery } from "@tanstack/react-query";
import { DollarSign, Loader2 } from "lucide-react";

export function ScHillsRevenueCard() {
    const { data: monthReceipt, isFetching: isLoadingMonthReceipt } = useQuery({
    queryKey: ['hills', 'faturado-card'],
    queryFn: getHillsReceipt,
  })

  const monthReceiptRs = monthReceipt?.find(receipt => receipt.estado === 'SC1');
    return (
        <Card>
            <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-base font-semibold">Receita total Hills SC (mês)</CardTitle>
